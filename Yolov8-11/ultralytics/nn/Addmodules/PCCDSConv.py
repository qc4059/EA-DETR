...
# # DSConv类：
# # 残差结构：添加了self.residual，若输入输出通道数不同，就使用 1x1 卷积；若相同，则使用恒等映射。在forward方法里，把残差加到最终输出上。
# # 通道注意力机制（SE 模块）：添加了SEModule类，该类实现了通道注意力机制。在forward方法中，将其应用于卷积输出。
# # Bottleneck_DySnakeConv类：
# # 门控融合（Gated Fusion）：添加了GatedFusion类，该类实现了门控融合。在forward方法中，用门控融合替代简单的特征拼接。
# # C3k2_PCCDSConv类：
# # 随机深度（DropPath）：添加了DropPath类，该类实现了随机深度。在__init__方法中初始化，在forward方法中应用。
...
#     def __init__(self, inc, ouc, k=3, radius_elastic=0.8) -> None:  # 添加 radius_elastic 参数
#         super().__init__()

#         self.conv_0 = Conv(inc, ouc, k)
#         self.conv_x = DSConv(inc, ouc, 0, k, radius_elastic=radius_elastic)  # 显式传入参数
#         self.conv_y = DSConv(inc, ouc, 1, k, radius_elastic=radius_elastic)  # 显式传入参数
...
#     def __init__(self, in_ch, out_ch, morph, kernel_size=3, if_offset=True, extend_scope=0.5, radius_elastic=0.8):
#         """
...
#         :param radius_elastic: the elastic coefficient of radius
#         """
...
#         # 调整分组数量，确保 out_ch 能被分组数量整除
#         num_groups = min(out_ch, 4)
...
#         self.gate = nn.Conv2d(in_channels * 2, 1, kernel_size=1)
...
#         self.gated_fusion = GatedFusion(c2)

#     def forward(self, x):
#         """'forward()' applies the YOLOv5 FPN to input data."""
#         conv1_out = self.cv1(x)
#         conv2_out = self.cv2(conv1_out)
#         conv3_out = self.cv3(conv2_out)
#         if self.add:
#             fused = self.gated_fusion(x, conv3_out)
#             return fused
#         else:
#             return conv3_out
...
#     def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True, drop_path_prob=0.1):

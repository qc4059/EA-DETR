...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads=2, window_size=16, shift_size=0,
...
class Local_pred(nn.Module):
    def __init__(self, dim=16, number=2, type='ccc'):
        ...
        if type == 'ccc':
            ...
            blocks1 = [CBlock_ln(16, drop_path=0.01), CBlock_ln(16, drop_path=0.05)]
            blocks2 = [CBlock_ln(16, drop_path=0.01), CBlock_ln(16, drop_path=0.05)]
        elif type == 'ttt':
            blocks1, blocks2 = [block_t for _ in range(number)], [block_t for _ in range(number)]
        elif type == 'cct':
            blocks1, blocks2 = [block, block_t], [block, block_t]
        #    block1 = [CBlock_ln(16), nn.Conv2d(16,24,3,1,1)]
        self.mul_blocks = nn.Sequential(*blocks1, nn.Conv2d(dim, 3, 3, 1, 1), nn.ReLU())
        self.add_blocks = nn.Sequential(*blocks2, nn.Conv2d(dim, 3, 3, 1, 1), nn.Sigmoid())
...
class Local_pred_S(nn.Module):
    def __init__(self, in_dim=3, dim=16, number=2, type='ccc'):
        ...
        self.add_end = nn.Sequential(nn.Conv2d(dim, 3, 3, 1, 1), nn.Sigmoid())
        self.apply(self._init_weights)

...
our_class_labels = ('pedestrian', 'people', 'bicycle', 'car', 'van', 'truck', 'tricycle', 'awning-tricycle', 'bus', 'motor', )
...
class OurDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.classes = 'VisDrone2019-DET-train' if image_set == 'train' else 'VisDrone2019-DET-val' if image_set == 'val' else 'VisDrone2019-DET-test-dev'
        self.coco = COCO(os.path.join(self.data_dir, self.classes, self.json_file))
    ...
    def pull_image(self, index):
        ...
        img_file = os.path.join(
                self.data_dir, self.classes , 'images', im_ann["file_name"])
    ...
    def pull_anno(self, index):
        ...
        im_ann = self.coco.loadImgs(img_id)[0]  # 根据图片的id信息 获取 这张图片内 所有的 目标物 的标注ID
        anno_ids = self.coco.getAnnIds(imgIds=[int(img_id)], iscrowd=0)# 图片内 目标物的 唯一ID
        annotations = self.coco.loadAnns(anno_ids) # 根据这个唯一ID 获取 目标物的 信息
...
if __name__ == "__main__":
    ...
    # opt
    parser.add_argument('--root', default='E:/Learning/Data/VisDrone',
                        help='data root')

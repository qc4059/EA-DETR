...
class COCODataset(Dataset):
    ...
    def __init__(self, 
                 img_size=640,
                 data_dir=None, 
                 image_set='train2017',  # 'train2017' if is_train else 'val2017',
                 trans_config=None,
                 transform=None,
                 is_train=False,
                 load_cache=False):
    ...
    def pull_anno(self, index):
        ...
        anno_ids = self.coco.getAnnIds(imgIds=[int(img_id)], iscrowd=False) # 图片内 目标物的 唯一ID
        ...
        bboxes = []
        labels = []
        for anno in annotations:  # 这个循环 是获取 目标物的 信息
            if 'bbox' in anno and anno['area'] > 0:
                # bbox
                x1 = np.max((0, anno['bbox'][0]))  # 边界框
                ...
                if x2 < x1 or y2 < y1:
                    continue
                # class label
                cls_id = self.class_ids.index(anno['category_id'])   # 类别
...
if __name__ == "__main__":
    ...
    # opt
    parser.add_argument('--root', default='E:/Learning/Data/VisDrone2',
                        help='data root')

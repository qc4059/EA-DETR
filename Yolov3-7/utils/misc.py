...
def build_dataloader(args, dataset, batch_size, collate_fn=None):
    ...
    # b = list(batch_sampler_train) 
    dataloader = DataLoader(dataset, batch_sampler=batch_sampler_train,
                            collate_fn=collate_fn, num_workers=args.num_workers, pin_memory=True)
        # 调试：迭代 dataloader 并打印每个批次内容
    # print("Dataloader contents:")
    # for i, (images, targets) in enumerate(dataloader):
    #     print(f"Batch {i}:")
    #     print(f"Images shape: {images.shape}")  # 图像的形状
    #     print(f"Targets: {targets}")           # 标签内容
    #     if i >= 2:  # 打印前 3 个批次即可，避免输出过多内容
    #         break
    return dataloader
...
def compute_flops(model, img_size, device):
    ...
    print('==============================')
    flops, params = profile(model, inputs=(x, ), verbose=False)   # 这里是什么？

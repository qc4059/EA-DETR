...
class Yolov8Trainer(object):
    ...
    def __init__(self, args, data_cfg,    model_cfg,    trans_cfg, device, model,       criterion, world_size):
        #                                                          这里的 model 和 criterion 各是 框架的实例化 和 计算损失类的实例化
        # ------------------- 基础参数 -------------------
        self.args          = args
        self.epoch         = 0
        self.best_map      = -1.
        self.last_opt_step = 0
        self.no_aug_epoch  = args.no_aug_epoch   # 20  表示最后20个epoch取消 数据增强
        self.clip_grad     = 10
        self.device        = device  # cuda
        self.criterion     = criterion  # 计算损失的类
        ...
        ## 构建训练(Train)所需的数据预处理
        #                                                  通用参数              数据集参数                  32                                  True
        self.train_transform, self.trans_cfg = build_transform(args=args, trans_config=self.trans_cfg, max_stride=model_cfg['max_stride'], is_train=True)
        # 这个train_transform 是数据预处理 类的实例化

        ## 构建测试(Evaluate)所需的数据预处理    和 训练一样
        self.val_transform, _ = build_transform( args=args, trans_config=self.trans_cfg, max_stride=model_cfg['max_stride'], is_train=False)
        ...
        # self.dataset 是VocDetection类的 实例化
    ...
    def train(self, model):
        for epoch in range(self.start_epoch, self.args.max_epoch):
            ...
            # +++ 每5个epoch保存一次模型的逻辑 +++
            if (epoch + 1) % 5 == 0:  # 注意：epoch通常从0开始，所以用epoch+1
                # 构建包含epoch数的文件名
                weight_name = '{}_epoch_{}.pth'.format(self.args.model, epoch + 1)
                checkpoint_path = os.path.join(self.path_to_save, weight_name)
                
                print('Saving model at epoch {}'.format(epoch + 1))
                torch.save({
                    'model': model.state_dict(),
                    'mAP': round(self.evaluator.map*100, 1) if hasattr(self, 'evaluator') else 0.0,
                    'optimizer': self.optimizer.state_dict(),
                    'epoch': epoch,
                    'args': self.args
                }, checkpoint_path)
    ...
    def eval(self, model, epoch):
        ...
        if distributed_utils.is_main_process():
            ...
            # 如果Evaluator类不是None，则进行测试
                ...
                # 测试模型的性能
                with torch.no_grad():
                    self.evaluator.evaluate(model_eval,epoch=epoch)
    ...
    def train_one_epoch(self, model):
        # 一些基础参数
        epoch_size = len(self.train_loader)  # Dataset size: 16551    Batch size: 16    Epoch size: 1034
        ...
        # 训练模型一个epoch
        for iter_i, (images, targets) in enumerate(self.train_loader):  # targets 表示 一个批次的样本图片中的 标注信息 包含16张图片中的所有目标信息。
            # print(targets)
...
# YOLOX Trainer: 主要用于训练YOLOX和本项目实现的较为简单的YOLOv7
class YoloxTrainer(object):
    def __init__(self, args, data_cfg, model_cfg, trans_cfg, device, model, criterion, world_size):
        ...
        # ---------------------------- YOLOX项目的超参数设置 ----------------------------
        self.optimizer_dict = {'optimizer': 'sgd',    # 使用SGD优化器; 若使用AdamW，则修改为'adamw'，但建议使用SGD优化器
                               'momentum': 0.9,       # SGD所需的momentum，AdamW优化器不需要此参数
                               'weight_decay': 5e-4,  # SGD所需的weight decay; 若使用AdamW，则修改为5e-2
                               'lr0': 0.01,           # 初始学习率; 若使用AdamW，则修改为0.001
                               }
        ## 学习策略的超参数
        self.lr_schedule_dict = {'scheduler': 'cosine', # 使用YOLOX官方的Cosine衰减策略
    ...
    def train(self, model):
        for epoch in range(self.start_epoch, self.args.max_epoch):
            ...
            # 检查当前是否进入训练的第三阶段，在第三阶段中，会所有的数据增强
            ...
            # 在每个 epoch 结束后保存
            if epoch % 1 == 0:
                weight_name = f"{self.args.model}_epoch_{epoch}.pth"
                checkpoint_path = os.path.join(self.path_to_save, weight_name)
                torch.save({
                    'model': model.state_dict(),
                    'optimizer': self.optimizer.state_dict(),
                    'epoch': epoch,
                    'mAP': self.evaluator.map,
                    'args': self.args
                }, checkpoint_path)
    ...
    def check_second_stage(self):
        # 在第二阶段，关闭Mosaic和Mixup两个强大的数据增强
        # 在YOLOX官方中，平移、缩放和剪裁等弱增强在第二阶段也会被关闭
        # 笔者实现的第二阶段会继续使用平移、缩放和剪裁等弱增强
        # 因此，笔者实现的第二阶段要略不同于官方的YOLOX
        print('============== Second stage of Training ==============')
        self.second_stage = True

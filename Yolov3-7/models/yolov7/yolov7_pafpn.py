...
class Yolov7PaFPN(nn.Module):
    def __init__(self, 
                 in_dims=[512, 1024, 512],  # [128,256,256]
                 out_dim=None,     # 128
                 channel_width : float = 1.0,   # 0.5
                 branch_width  : int   = 4.0,   # 2
                 branch_depth  : int   = 1.0,    # 1
                 act_type='silu',
                 norm_type='BN',
                 depthwise=False):
        ...
        c3, c4, c5 = self.fpn_dims  # 128,256,256
...
#torch.Size([1, 128, 80, 80])  [1, 256, 40, 40] [1, 256, 20, 20]
        c3, c4, c5 = features 
        # Top down
        ## P5 -> P4
        c6 = self.reduce_layer_1(c5)  # (1,128,20,20)
        c7 = F.interpolate(c6, scale_factor=2.0)  # (1,128,40,40)
        c8 = torch.cat([c7, self.reduce_layer_2(c4)], dim=1) # (1,256,40,40)      self.reduce_layer_2(c4): (1,128,40,40)
...
def build_fpn(cfg, in_dims, out_dim=None):
    ...
    if model == 'yolov7_pafpn':
        fpn_net = Yolov7PaFPN(in_dims       = in_dims,   # [128,256,256]                   
                              out_dim       = out_dim,    # 128
                              channel_width = cfg['channel_width'],  # 0.5
                              branch_width  = cfg['branch_width'],   # 2
                              branch_depth  = cfg['branch_depth'],  # 1
                              act_type      = cfg['fpn_act'],  # SiLU
                              norm_type     = cfg['fpn_norm'],  # BN
                              depthwise     = cfg['fpn_depthwise']  # False
                              )
    else:

...
class Conv(nn.Module):
    def __init__(self, 
        ...
        # 构建普通的标准卷积
            ...
            convs.append(get_conv2d(c1, c2, k=k, p=p, s=s, d=d, g=1, bias=add_bias)) # 这个列表添加的是 函数返回的对象 nn.Conv2d....
            if norm_type:
                convs.append(get_norm(norm_type, c2))
            if act_type:
                convs.append(get_activation(act_type))
            
        self.convs = nn.Sequential(*convs)  # 关于 Sequential 按顺序组合 nn.Module模块 *convs 是一个解包操作，将列表中的元素 按照顺序作为参数传入Sequential
...
class ELANBlock(nn.Module):
    #                   64        64                 这个是类型注释，也可以用 branch_depth = 2
    def __init__(self, in_dim, out_dim, squeeze_ratio=0.5, branch_depth :int=2, act_type='silu', norm_type='BN', depthwise=False):
        super(ELANBlock, self).__init__()
        inter_dim = int(in_dim * squeeze_ratio)  # 32
        self.cv1 = Conv(in_dim, inter_dim, k=1, act_type=act_type, norm_type=norm_type) # 64->32   160 x 160 x 32
        self.cv2 = Conv(in_dim, inter_dim, k=1, act_type=act_type, norm_type=norm_type) # 64->32   160 x 160 x 32
        self.cv3 = nn.Sequential(*[
            #        32        32
            Conv(inter_dim, inter_dim, k=3, p=1, act_type=act_type, norm_type=norm_type, depthwise=depthwise)  # 160 x 160 x 32
            for _ in range(round(branch_depth))  # range 生成序列 round 四舍五入为整数
        ])
...
class ELANBlockFPN(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, squeeze_ratio=0.5, branch_width :int=4, branch_depth :int=1, act_type='silu', norm_type='BN', depthwise=False):
        ...
        # Basic parameters
        inter_dim = int(in_dim * squeeze_ratio)   # 128
        inter_dim2 = int(inter_dim * squeeze_ratio)  #  64
        # Network structure
        #                 256     128
        self.cv1 = Conv(in_dim, inter_dim, k=1, act_type=act_type, norm_type=norm_type)   
        self.cv2 = Conv(in_dim, inter_dim, k=1, act_type=act_type, norm_type=norm_type)
        self.cv3 = nn.ModuleList()
        for idx in range(round(branch_width)):  # branch_width = 2
            if idx == 0:
                #             128           64
            ...
            # deeper
            if round(branch_depth) > 1:  # branch_depth = 1
    ...
    def forward(self, x):  # x : (1, 256, 40, 40)
        x1 = self.cv1(x)    #   (1, 128, 40, 40)
        x2 = self.cv2(x)     # (1, 128, 40, 40)
        inter_outs = [x1, x2]
        for m in self.cv3:     # len(cv3) = 2
            y1 = inter_outs[-1]
            y2 = m(y1)                # (128,40,40) (128,40,40) (64,40,40) (64,40,40)
            inter_outs.append(y2)   # inter_outs = [x1, x2, y2 ,y2]
        out = self.out(torch.cat(inter_outs, dim=1))    # (384,40,40)->(128,40,40)
        # print(torch.cat(inter_outs, dim=1).size())
        # print(out.size())
        return out

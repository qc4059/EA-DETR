...
class YOLOX(nn.Module):
    def __init__(self, 
        ...
        self.obj_preds = nn.ModuleList(
                            [nn.Conv2d(head.reg_out_dim, 1, kernel_size=1)    #这里表明的是将输入维度从 256 变到 1 表每个网格内的通道数
                                for head in self.non_shared_heads
                              ]) 
        self.cls_preds = nn.ModuleList(
                            [nn.Conv2d(head.cls_out_dim, self.num_classes, kernel_size=1)       # 256 -> 20 类别参数深度
                                for head in self.non_shared_heads
                              ]) 
        self.reg_preds = nn.ModuleList(
                            [nn.Conv2d(head.reg_out_dim, 4, kernel_size=1)      # 256 -> 4 位置参数深度
                                for head in self.non_shared_heads
                              ])                 
    ...
    def forward(self, x):
        if not self.trainable:
            ...
            for level, (feat, head) in enumerate(zip(pyramid_feats, self.non_shared_heads)):
                ...
                B, _, H, W = cls_pred.size()  # 16,20,80,80
                fmp_size = [H, W]  # [80,80]

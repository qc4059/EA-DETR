...
class Criterion(object):
    def __init__(self,
        ...
        # matcher
        matcher_config = cfg['matcher']    # 'matcher': {'center_sampling_radius': 2.5, 'topk_candicate': 10},
        self.matcher = SimOTA(
    ...
    def __call__(self, outputs, targets, epoch=0):        
        """
            ...
            targets: (List) [dict{'boxes': [...], 
                                 ...
                                 'orig_size': ...}, ...]
        """
        # print(len(targets)) # 看一下targets长度   应该是包含一张图片中，所有目标物的信息
        # print(len(targets[0]))   # 这个targets呢，是这样的，它是一个批次的图片中所有目标物的信息，targets[0]表示第一张图片中所有目标物的信息（用三个变量进行表示，分别是各个目标物的boxs尺寸、标签类别、原始图片大小尺寸）
        # print(targets[0])
        bs = outputs['pred_cls'][0].shape[0]   # outputs['pred_cls'][0].shape =  torch.Size([16, 6400, 20])  outputs['pred_cls']一共存储了3个尺度的类别预测信息 分别为 torch.Size([16, 6400, 20])  torch.Size([16, 1600, 20])  torch.Size([16, 400, 20]) 
        device = outputs['pred_cls'][0].device
        fpn_strides = outputs['strides']  # [8,16,32]
        anchors = outputs['anchors']   # anchors 类型是 list  里面存储了 三个tensor类型 [torch.Size([6400, 2]),torch.Size([1600, 2]),torch.Size([400, 2])]
        # print(type(anchors[0]))   # torch.tensor
        # print(anchors[0].size())  #  [6400,2]
        # preds: [B, M, C]   
        obj_preds = torch.cat(outputs['pred_obj'], dim=1)
        cls_preds = torch.cat(outputs['pred_cls'], dim=1)     # 将三个尺度信息整合至 一个[16,8400,20]
        ...
        for batch_idx in range(bs):
            tgt_labels = targets[batch_idx]["labels"].to(device)   # 遍历每张图片中的 所有目标物的 标签信息
            tgt_bboxes = targets[batch_idx]["boxes"].to(device)   # 遍历每张图片中的 所有目标物的 边界框信息
            ...
            if len(tgt_labels) == 0 or tgt_bboxes.max().item() == 0.:
                ...
                (
                    ...
                    fpn_strides = fpn_strides,   # [8,16,32]
                    anchors = anchors,     #
                    pred_obj = obj_preds[batch_idx],  # [8400,1]
                    pred_cls = cls_preds[batch_idx],  # [8400,20]
                    pred_box = box_preds[batch_idx],  # [8400,4]
                    tgt_labels = tgt_labels,  # 每张图片中的 所有目标物的 标签信息 （真实值）
                    tgt_bboxes = tgt_bboxes  # 每张图片中的 所有目标物的 边界框信息 （真实值）
                    )

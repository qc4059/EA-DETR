...
class Yolov5Matcher(object):
    ...
    def __call__(self, fmp_sizes, fpn_strides, targets):
        ...
        bs = len(targets)
        gt_objectness = [
            torch.zeros([bs, fmp_h, fmp_w, self.num_anchors, 1]) # （16,80,80,3,1）（16,40,40,3,1）（16,20,20,3,1）
            for (fmp_h, fmp_w) in fmp_sizes
            ]
        gt_classes = [
            torch.zeros([bs, fmp_h, fmp_w, self.num_anchors, self.num_classes])  #（16,80,80,3,80）（16,40,40,3,80）（16,20,20,3,80）
            for (fmp_h, fmp_w) in fmp_sizes
            ]
        gt_bboxes = [
            torch.zeros([bs, fmp_h, fmp_w, self.num_anchors, 4]) # （16,80,80,3,4）（16,40,40,3,4）（16,20,20,3,4）
            for (fmp_h, fmp_w) in fmp_sizes
            ]

        for batch_index in range(bs):   # 遍历图片
            ...
            for gt_box, gt_label in zip(tgt_box, tgt_cls):  # 遍历图片中的 目标物
                # get a bbox coords

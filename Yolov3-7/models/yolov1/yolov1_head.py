...
class DecoupledHead(nn.Module):
    ...
    def __init__(self, cfg, in_dim, out_dim, num_classes=80):
        ...
        self.num_cls_head=cfg['num_cls_head']   #  2  这个参数 是 控制 类别检测头 中的 卷积层数的
        self.num_reg_head=cfg['num_reg_head']   #  2  这个参数 是 控制 回归检测头 中的 卷积层数的
        self.act_type=cfg['head_act']     #   lrelu
        self.norm_type=cfg['head_norm']   #    BN

        # ------------------ 类别检测头 ------------------
        cls_feats = []
        self.cls_out_dim = max(out_dim, num_classes)   # 类别数跟 输出维度作比较，为什么
        for i in range(cfg['num_cls_head']):  # 0，1
            if i == 0:
                cls_feats.append(
                    #       512            512 

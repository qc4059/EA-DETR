...
class YoloMatcher(object):
    ...
    def __call__(self, fmp_size, stride, targets):
        ...
        """
        # 准备后续处理会用到的变量
        bs = len(targets)   # bs 表示 一批图片数据 的 数量
        fmp_h, fmp_w = fmp_size  # 网络预测的最后一层 特征图的 宽度和高度
        gt_objectness = np.zeros([bs, fmp_h, fmp_w, 1])  # （16,20,20,1）
        gt_classes = np.zeros([bs, fmp_h, fmp_w, self.num_classes])  # （16,20,20,20）
        gt_bboxes = np.zeros([bs, fmp_h, fmp_w, 4]) # （16,20,20,4）

        # 第一层for循环遍历每一张图像的标签
        for batch_index in range(bs):
            targets_per_image = targets[batch_index]  #targets 表示 一个批次的样本图片中的 标注信息 包含16张图片中的所有目标信息。（是列表）
            # [N,]
            ...
            for gt_box, gt_label in zip(tgt_box, tgt_cls):  # 'boxes': tensor([[234.,   0., 559., 303.]]),    'labels': tensor([2.])
                # 获得该目标的边界框坐标
                x1, y1, x2, y2 = gt_box

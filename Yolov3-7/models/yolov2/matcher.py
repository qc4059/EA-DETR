...
class Yolov2Matcher(object):
    def __init__(self, iou_thresh, num_classes, anchor_size):
        ...
        self.iou_thresh = iou_thresh    # 0.5
        # anchor box
        self.num_anchors = len(anchor_size)   # 5
        self.anchor_size = anchor_size

        self.anchor_boxes = np.array( [[0., 0., anchor[0], anchor[1]] for anchor in anchor_size] )  # [KA, 4]

# [[  0.   0.  38.  64.]
#  [  0.   0.  89. 147.]
#  [  0.   0. 145. 285.]      anchor_boxes
#  [  0.   0. 258. 169.]
#  [  0.   0. 330. 340.]]
    ...
    def __call__(self, fmp_size, stride, targets):
        ...
        fmp_h, fmp_w = fmp_size  # 13 13 
        gt_objectness = np.zeros([bs, fmp_h, fmp_w, self.num_anchors, 1])  #  [16, 13, 13, 5, 1]
        gt_classes = np.zeros([bs, fmp_h, fmp_w, self.num_anchors, self.num_classes])  # [16, 13, 13, 5, 20]
        gt_bboxes = np.zeros([bs, fmp_h, fmp_w, self.num_anchors, 4]) #  [16, 13, 13, 5, 4]
        ...
        for batch_index in range(bs):
            ...
            for gt_box, gt_label in zip(tgt_box, tgt_cls):
                # 获得该目标的边界框坐标
                x1, y1, x2, y2 = gt_box # eg:  262, 210, 323, 338

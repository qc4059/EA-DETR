...
class Criterion(object):
    ...
    def __call__(self, outputs, targets, epoch=0):
        ...
        # ------------------ 标签分配 ------------------
        gt_objectness, gt_classes, gt_bboxes = self.matcher(fmp_size = fmp_size,  # (20,20)
                                                            stride   = stride,  # 32
                                                            targets  = targets)
        # 将标签的shape处理成和预测的shape相同的形式，以便后续计算损失
        print(gt_objectness.size())    # (16,2000,1)
        gt_objectness = gt_objectness.view(-1).to(device).float()               # [BM,]
        print(gt_objectness.size())   # (32000)

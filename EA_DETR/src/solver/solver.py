...
class BaseSolver(object):
    def __init__(self, cfg: BaseConfig) -> None:
        ...
        self.cfg = cfg   # self.cfg 也是 实例

    def setup(self, ):
        '''Avoid instantiating unnecessary classes 
        '''
        cfg = self.cfg   # cfg 依然是实例
        device = cfg.device  # 实例调用参数
        self.device = device
        self.last_epoch = cfg.last_epoch

        # self.model = dist.warp_model(cfg.model.to(device), cfg.find_unused_parameters, cfg.sync_bn)  # 可设置并行GPU训练 
        self.model = cfg.model.to(device)
        self.criterion = cfg.criterion.to(device)  # 实例调用 函数

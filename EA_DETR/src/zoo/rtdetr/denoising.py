...
def get_contrastive_denoising_training_group(targets,
                                             num_classes,  # 80
                                             num_queries,  # 300
                                             class_embed,   # Embedding(81,256,padding_idx = 80)
                                             num_denoising=100,
    ...
    # 获取到 各个图片的 目标物的个数 eg:[37,0,1,19]
    num_gts = [len(t['labels']) for t in targets]  
    device = targets[0]['labels'].device
    
    max_gt_num = max(num_gts) # 找到四张图片中 目标物最多的那个，记录此图片中目标物的个数
    if max_gt_num == 0:
        return None, None, None, None
    # num_group 的用处是什么
    num_group = num_denoising // max_gt_num  # 2
    num_group = 1 if num_group == 0 else num_group
    # pad gt to max_num of a batch
    bs = len(num_gts)   # 4 图片的数量
    ...
    for i in range(bs):
        num_gt = num_gts[i]
        if num_gt > 0:
            # 取出第一张图片的标签，并将其存入input_query_class
            input_query_class[i, :num_gt] = targets[i]['labels'] # （4,37）
            input_query_bbox[i, :num_gt] = targets[i]['boxes'] #(4,37,4)
            pad_gt_mask[i, :num_gt] = 1 #(4,37)
    # each group has positive and negative queries.
    # 重复 2*num_group遍 （4，148）
    ...
    # contrastive denoising training positive index   squeeze(-1)去除最后一个维度

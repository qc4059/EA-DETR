...
# 自动注册 这个还挺怪，功能就是 在其它文件下定义的 类或者函数 只要调用了@register 就可以自动填充到 全局变量下 通过调用全局变量的 键就可以调用函数 类的话 需要额外create（）
def register(cls: type):
    ...
    if inspect.isfunction(cls):
        GLOBAL_CONFIG[cls.__name__] = cls  # cls.__name__ 表示提取 函数名 或者 类名
...
def create(type_or_name, **kwargs):
    ...
    cfg = GLOBAL_CONFIG[name]  # 返回的就是一个字典，关于RT-DETR中的配置说明，比如backbone：PResnet
...
def load_config(file_path, cfg=dict()):
    ...
    with open(file_path) as f:
        ...
        if file_cfg is None:   # 返回的这个file_cfg 是一个字典
            return {}
...
def merge_config(config, another_cfg=None):  # 合并参数的过程，就是修改参数的过程
    """
